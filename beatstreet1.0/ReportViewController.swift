...
class ReportViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, CLLocationManagerDelegate, UITextFieldDelegate{
...
//    let usersRef = Database.database().reference(withPath: "online")
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.wardInput.delegate = self
        self.mobileNumber.delegate = self
    }
    
    // in order to hide the keybord when the background is touched
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        self.view.endEditing(true)
    }
    
    // in order to hide the keybord when the return button is clicked
